...
**[표 6-1] 하이브리드 추론 시간 상세 분해**

| 구분 | 단계 (Stage) | 시간 (ms) | 점유율 (%) | 비고 |
| --- | --- | --- | --- | --- |
| **Pre-task** | `pre_total` | 0.70 | 4.1% | LB + Pack |
| **Data Move** | `h2d` | 0.22 | 1.3% | Host to Device |
| **DLA Segment** | **`A/infer`** | **9.25** | **53.9%** | **Backbone (INT8)** |
| **Sync** | `wait` | 0.002 | 0.0% | 동기화 오버헤드 미미 |
| **GPU Segment** | **`B`** | **6.86** | **39.9%** | **C2PSA+Heads (FP16)** |
| **Post-task** | `post` | 1.50 | 0.8% | D2H + Decode/NMS |
| **Total** | **`e2e`** | **17.17** | **100%** | **p50 지표** |
...
* [TIMING]/power 측정 체계도 이 시기에 같이 고도화
...
## Appendix B. PTQ 붕괴 원인 규명 경과 및 재현(요약)
...
## Appendix C. Hybrid 엔진 구성 및 실행(Implementation)
...
## Appendix D. Reproducibility: End-to-end 재현 절차(1~5)
...
```bash
./bench --mode=gpu [yolo11m_fp16.plan] [your/valid/set/dir] \
  --sched=throughput \
  --conf=0.001 --iou=0.45 --max_det=1000 \
...
## Appendix E. KL 최적점 근사 재현: Histogram → KL-sweep → Zigzag 지표
...
### E.3 KL-sweep(근사): q_T 구성 및 KL 계산
...
### E.4 KL(T) 톱니 성분(zigzag) 정량화 지표

#### E.4.1 TV_norm (Total Variation, 정규화)
...
#### E.4.2 HFE_norm (High-Frequency Energy, 정규화)

* 2차 차분: $d2(t) = KL(t+1) - 2*KL(t) + KL(t-1)$
* $HFE = Mean(d2^2(t))$
* $HFE_{norm} = HFE / (Var^2(KL) + ε)$ 또는 $HFE / (Mean^2(KL) + ε)$

해석: `HFE_norm`이 클수록 고주파(톱니) 성분이 크다.

### E.5 그림: KL(T) overlay (const vs mean vs random)

**Figure E-1. KL(T) curve overlay with best_Thr markers (abs hist range=[0,32], bins=2048)**
