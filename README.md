...
S21+ 및 S25 Ultra 환경에서 추론 실험을 진행한 결과, GPU delegate를 적용한 실행 경로에서는 모델 그래프 전체가 GPU로 위임되어 실행되었으며 반복 실행 과정에서도 안정적인 추론이 가능했습니다. 반면 NPU를 포함한 실행 경로는 기기 및 환경에 따라 사용 가능한 범위가 제한적이었습니다.

특히 가속기 할당 방식과 실행 경로를 사용자가 명확히 지정할 수 없어, 가속기 간 분할 처리를 포함한 하이브리드 추론 구조에 대한 실험을 구성하는 것 자체가 사실상 불가능했습니다. 이로 인해 하이브리드 추론 구조에 대한 실험적 검증 역시 진행할 수 없는 상태였습니다.
...
모바일 환경에서는 가속기 지정과 내부 동작을 명확히 확인하는 데 제약이 존재하여 하이브리드 분할 추론에 대한 실험을 진행하기 어려웠습니다.

이에 따라 이후 단계에서는 실험 조건을 보다 명확히 설정할 수 있는 환경인 **Jetson Orin Nano 플랫폼**에서 동일한 주제를 이어서 살펴보고자 합니다. 해당 환경에서는 GPU 실행 경로를 기준으로 추론이 실제 환경에서 어떤 성능을 보이는지를 확인하는 것부터 단계적으로 진행할 예정입니다.
